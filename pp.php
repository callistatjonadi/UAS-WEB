...
$user_id = $_SESSION['user_id'];
  ...
  if(isset($_POST['save'])){
    if(isset($_FILES['profile']) && $_FILES['profile']['size'] > 0) { //kalau ada file yg diupload
    $profile_name = $_FILES['profile']['name'];
    $profile_path = "media/" . $profile_name;

    move_uploaded_file($_FILES['profile']['tmp_name'], $profile_path); //video di temporary store ke videopath

    $sql = "UPDATE student SET profile = ? WHERE id = '$user_id'";
...
$sql = "SELECT * FROM student WHERE id = '$user_id'"; //biar kalau g update pn ttp ada
$stmt = $conn->prepare($sql);
$stmt->execute();
...
<nav class="navbar navbar-expand-lg bg-body-tertiary">
    ...
    <a class="nav-link" href="home.php"><img class="navbar-brand studee" src="gambar/studee.png" width="110" height="50"></a>
      ...
      <div class="text">
      <a href="setting.php"><img class="profil" style="border-radius: 50%;" src="media/<?php echo $profile; ?>"></a>
  ...
  <div class="row">
    ...
    <main class="col-md-9 ms-sm-auto col-lg-10 px-md-4">
      <div class="d-flex justify-content-between flex-wrap flex-md-nowrap align-items-center pt-3 pb-2 mb-3 border-bottom" style="margin-left:60px;">
        <h1 class="h2">Profile Picture</h1>
      </div>

      <div>
        <h5 style="margin-left:60px; margin-top:30px;">Profile Picture Sekarang</h5>
      </div>  

      
      <div class="kotakpp" style="margin-left:60px; margin-top:10px;">
      <?php if(isset($_SESSION['profile'])){ ?>
        <img src="media/<?php echo $profile; ?>" style="width: 200px; height: 200px; border-radius: 50%; margin-left: 200px;">
      <?php } ?>
      <?php if($_SESSION['profile'] == "") { ?>
        <img src="gambar/profile.png" class="gambarpp">
      <?php } ?>
      </div>
      <form method="POST" enctype="multipart/form-data" action="">
      <div class="mb-3">
        <h5 for="formFile" class="form-label" style="margin-left:60px; margin-top:20px;">Ganti Profile Picture</h5>
        <input class="form-control" type="file" id="formFile" style="margin-left:60px; margin-top:10px; width:500px;" name="profile">
      </div>  

      <div><button class="button5" style="margin-left:60px; margin-top:20px;" name="save">Save</button></div>
      </form>
